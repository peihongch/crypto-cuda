...
namespace cuda
{
    ...
    #define SHA384_512AddLength(context, length)                            \
        (addTemp[3] = (length), SHA512_ADDTO4((context)->Length, addTemp),  \
         (context)->Corrupted =                                             \
             (((context)->Length[3] == 0) && ((context)->Length[2] == 0) && \
    ...
    __constant__ static const uint32_t K[80 * 2] = {
    ...
    __device__ int SHA384_512ResultN(SHA512Context* context, uint8_t Message_Digest[], int HashSize);
    ...
    /* Constants defined in FIPS-180-2, section 4.2.3 */
    __constant__ static const uint64_t K[80] = {
     ...
     *
     * */
    __device__ int SHA384Input(SHA384Context* context, const uint8_t* message_array, unsigned int length)
    {
     ...
     *
     */
    __device__ int SHA384FinalBits(SHA384Context* context, const uint8_t message_bits, unsigned int length)
    {
     ...
     *
     */
    __device__ int SHA384Result(SHA384Context* context, uint8_t Message_Digest[SHA384HashSize])
    {
     ...
     * *
     */
    __device__ int SHA512Input(SHA512Context* context, const uint8_t* message_array, unsigned int length)
    {
    #ifdef USE_32BIT_ONLY
     ...
     *
     */
    __device__ int SHA512FinalBits(SHA512Context* context, const uint8_t message_bits, unsigned int length)
    {
    #ifdef USE_32BIT_ONLY
        static uint32_t addTemp[4] = {0, 0, 0, 0};
    #else
     ...
     *
     */
    __device__ int SHA512Result(SHA512Context* context, uint8_t Message_Digest[SHA512HashSize])
    {
