...
const uint8_t HASH_VALUES[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
                                0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

// Initialize array of round constants:
// (first 32 bits of the fractional parts of the cube roots of the first 64
// primes 2..311):
const uint8_t ROUND_CONSTANTS[64] = {
...
#define padding_length(len) (((len)*8 + 1 + 64 + 511) / 512 * 512 / 8)
#define add_module(n) ((n) & ((1 << 32) - 1))
#define u32_to_be(n)                                          \
...
#define be_to_u32(n)                                          \
    ((((n) & (0xff)) << 24) | ((((n) >> 8) & (0xff)) << 16) | \
     ((((n) >> 16) & (0xff)) << 8) | (((n) >> 24) & (0xff)))
#define be32_right_rotate(a, n) \
    (((((1 << (n)) - 1) & a) << (32 - (n))) | ((a) >> (n)))

#define CHUNK_SIZE (512 / 8)

void sha256(uint8_t* msg, uint64_t len, uint8_t* digest);
...
void sha256(uint8_t* msg, uint64_t len, uint8_t* digest) {
    ...
    padded_msg[len] = (1 << 7);
    padded_msg[padded_len - 1] = (len * 8) & 0xff;
    padded_msg[padded_len - 2] = ((len * 8) >> 8) & 0xff;
    padded_msg[padded_len - 3] = ((len * 8) >> 16) & 0xff;
    padded_msg[padded_len - 4] = ((len * 8) >> 24) & 0xff;
    padded_msg[padded_len - 5] = ((len * 8) >> 32) & 0xff;
    padded_msg[padded_len - 6] = ((len * 8) >> 40) & 0xff;
    padded_msg[padded_len - 7] = ((len * 8) >> 48) & 0xff;
    padded_msg[padded_len - 8] = ((len * 8) >> 56) & 0xff;
    ...
    for (i = 0; i < padded_len; i += CHUNK_SIZE) {
        ...
        // expect: 01101000011001010110110001101100
        // actual: 01101100011011000110010101101000

        // Extend the first 16 words into the remaining 48 words w[16..63] of
        // the message schedule array:
        for (j = 16; j < 64; j++) {
            s0 = be32_right_rotate(u32_to_be(w[j - 15]), 7) ^
                 be32_right_rotate(u32_to_be(w[j - 15]), 18) ^
                 (u32_to_be(w[j - 15]) >> 3);
            s1 = be32_right_rotate(u32_to_be(w[j - 2]), 17) ^
                 be32_right_rotate(u32_to_be(w[j - 2]), 19) ^
                 (u32_to_be(w[j - 2]) >> 10);
            w[j] = add_module(u32_to_be(w[j - 16]) + s0 + u32_to_be(w[j - 7]) +
                              s1);
        }
