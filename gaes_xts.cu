...
int main(int argc, char const *argv[]) {
    uint8_t ret_text[TEXT_SIZE] = {1};
    uint8_t *dev_ret_text;

    uint8_t text[TEXT_SIZE] = {0};
    uint8_t *dev_text;

    uint8_t cipher_text[TEXT_SIZE] = {1};
    uint8_t *dev_cipher_text;

    uint8_t key[KEY_LENGTH] = {0x0f, 0x15, 0x71, 0xc9, 0x47, 0xd9, 0xe8, 0x59,
    ...
    cudaMalloc((void **) &dev_ret_text, sizeof(ret_text));
    cudaMalloc((void **) &dev_text, sizeof(text));
    cudaMalloc((void **) &dev_cipher_text, sizeof(cipher_text));
    cudaMalloc((void **) &dev_key, sizeof(key));
    cudaMemcpy(dev_ret_text, ret_text, sizeof(ret_text),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_text, text, sizeof(text), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_cipher_text, cipher_text, sizeof(cipher_text),
               cudaMemcpyHostToDevice);
    ...
    for (i = 0; i < sizeof(key_bit) / sizeof(key_bit[0]); ++i) {
        xts_encrypt<<<dimGrid, dimBlock>>>(dev_key, key_bit[i] * 2, dev_cipher_text, dev_text,
                                           sizeof(text), 0);
        xts_decrypt<<<dimGrid, dimBlock>>>(dev_key, key_bit[i] * 2, dev_ret_text,
                                           dev_cipher_text, sizeof(cipher_text), 0);

        cudaMemcpy(ret_text, dev_ret_text, sizeof(ret_text),
                   cudaMemcpyDeviceToHost);
        cudaMemcpy(cipher_text, dev_cipher_text, sizeof(cipher_text),
                   cudaMemcpyDeviceToHost);
...
__device__ static void transport(uint8_t state[BLOCK_SIZE]) {
...
__device__ static void add_round_key(uint8_t state[BLOCK_SIZE],
...
__device__ static void _sub_bytes(uint8_t state[BLOCK_SIZE],
                                  const uint8_t *box) {
    int i;
    for (i = 0; i < BLOCK_SIZE; ++i)
        state[i] = box[state[i]];
...
__device__ static void _mix_columns(uint8_t state[BLOCK_SIZE],
...
__device__ static void aes_round(uint8_t state[BLOCK_SIZE],
                                 const uint8_t rk[BLOCK_SIZE]) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
...
__device__ static void aes_inv_round(uint8_t state[BLOCK_SIZE],
                                     const uint8_t inv_rk[BLOCK_SIZE]) {
...
__device__ static void aes_final_round(uint8_t state[BLOCK_SIZE],
                                       const uint8_t rk[BLOCK_SIZE]) {
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk);
...
__device__ static void inv_final_round(uint8_t state[BLOCK_SIZE],
                                       const uint8_t inv_rk[BLOCK_SIZE]) {
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, inv_rk);
...
__device__ void aes_encrypt(aes_context *ctx,
                            uint8_t cipher_text[BLOCK_SIZE],
                            const uint8_t text[BLOCK_SIZE]) {
    uint32_t nr = ctx->nr;
    uint32_t *rk = ctx->rk;
...
__device__ void aes_decrypt(aes_context *ctx,
                            ...
                            const uint8_t cipher_text[BLOCK_SIZE]) {
    uint32_t nr = ctx->nr;
...
__global__ void xts_encrypt(uint8_t *key,
                            ...
                            uint32_t len,
                            const uint64_t tweak) {
    unsigned int sectors = len / SECTOR_SIZE;
    unsigned int nblocks = SECTOR_SIZE / BLOCK_SIZE;
    uint8_t *t;
    const uint8_t *src;
    uint8_t *dst;
    unsigned int i, j;
    uint8_t tweak_buf[SECTOR_SIZE];

    aes_context *tweak_ctx = (aes_context *) malloc(sizeof(aes_context));
    aes_context *crypt_ctx = (aes_context *) malloc(sizeof(aes_context));

    aes_set_key(tweak_ctx, key, key_len / 2);
    aes_set_key(crypt_ctx, key + (key_len / 2) / sizeof(uint8_t), key_len / 2);

    for (i = 0; i < sectors; ++i) {
        /* calculate first value of T */
        *((uint64_t *) tweak_buf) = tweak + i;
        *(((uint64_t *) tweak_buf) + 1) = 0;
        aes_encrypt(tweak_ctx, tweak_buf, tweak_buf);

        j = 0;
        src = text + i * SECTOR_SIZE;
        dst = cipher_text + i * SECTOR_SIZE;
        goto first;

        for (j = 0; j < nblocks; j++) {
            gf128mul_x_ble(tweak_buf + j * BLOCK_SIZE, t);
            first:
            t = tweak_buf + j * BLOCK_SIZE;

            /* PP <- T xor P */
            be128_xor(dst + j * BLOCK_SIZE, t, src + j * BLOCK_SIZE);
        }
        ...
        /* C <- C xor CC */
        for (j = 0; j < nblocks; j++) {
            be128_xor(dst + j * BLOCK_SIZE, dst + j * BLOCK_SIZE, tweak_buf + j * BLOCK_SIZE);
        }
    }
}
...
__global__ void xts_decrypt(uint8_t *key,
    ...
    for (i = 0; i < sectors; ++i) {
        /* calculate first value of T */
        *((uint64_t *) tweak_buf) = tweak + i;
        *((uint64_t * )(tweak_buf + 8)) = 0;
        aes_encrypt(tweak_ctx, tweak_buf, tweak_buf);

        j = 0;
        src = cipher_text + i * SECTOR_SIZE;
        dst = text + i * SECTOR_SIZE;
        goto first;
